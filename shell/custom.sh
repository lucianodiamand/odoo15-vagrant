...
sed '/^addons_path/ s/$/,\/opt\/odoo15\/odoo-custom-addons\/employee_extended/' /etc/odoo15.conf > /tmp/odoo15.conf
mv /tmp/odoo15.conf /etc/odoo15.conf

systemctl stop odoo15

su $OE_USER -c "/opt/odoo15/odoo/odoo-bin -c /etc/odoo15.conf -d hcd -i employee_extended --without-demo=all --load-language es_AR --stop-after-init" 2> /dev/null
